...
(async function(){
  ...
  const uniqueVersionInfo = outputInfo.apkData.versionName + '('+ outputInfo.apkData.versionCode.toString() + ')-SC(' + revision+')';



  const variantFolderId= await getVariantFolderId(oAuth2Client,rootFolderId);  
  const fileNameToUse =  outputInfo.apkData.fullName + '-'+ uniqueVersionInfo + '.apk'
